...
//     async createWalletForUser(userId: string): Promise<{ publicKey: string, clientKeyShare: string }> {
...
//     async reconstructKeypair(userId: string, clientKeyShare: string): Promise<Keypair | null> {
//         const wallet = await this.walletRepository.findOne({ where: { userId } });
